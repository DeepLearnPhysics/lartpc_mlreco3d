...
@post_processing(data_capture=[],
                 ...
                 data_capture_optional=['input_data'],
                 result_capture_optional=['input_rescaled', 'cluster_label_adapted', 'truth_particles', 'truth_interactions'])
def find_cathode_crossers(data_dict, result_dict,
    ...
    for k in key_list:
        ...
        for i, p in enumerate(result_dict[k]):
            ...
            if p.units != 'cm':
                raise ValueError('Particle coordinates must be expressed in cm '
                    'to find cathode crossers, currently in {}'.format(p.units))
            ...
            # If the particle is composed of points from multiple contributing
            # TPCs in the same module, it is a cathode crosser
            ...
            if p.is_ccrosser and adjust_crossers and len(tpcs) == 2:
                # Adjust positions
                adjust_positions(result_dict, geo, i, truth=truth)

        # If we do not want to merge broken crossers, our job here is done
        if not merge_crossers:
            continue
        ...
        while i < len(candidate_ids):
            ...
            while j < len(candidate_ids):
                ...
                # Check that the particles live in adjacent TPCs of one module
                modules_j, tpcs_j = geo.get_contributors(pj.sources)
                ...
                compat &= dist_mat[pair_i, pair_j] < crossing_point_tolerance
                ...
                compat &= np.abs(offset_i + offset_j) < offset_tolerance
                ...
                compat &= np.arccos(cosang) < angle_tolerance

                # If compatible, merge
                if compat:
                    # Merge particle and adjust positions
                    adjust_positions(result_dict, geo, ci, cj, truth=truth)
        ...
        for ia in result_dict[int_k]:
            ...
            parts = [p for p in result_dict[k] if p.interaction_id == ia.id]
            for p in parts:
...
def adjust_positions(result_dict, geo, idx_i, idx_j=None,
        truth=False, truth_point_mode='points'):
    '''
    ...
    '''
    # If there is two indexes, start by creating a new merged particle object
    prefix = 'truth_' if truth else ''
    k, int_k = f'{prefix}particles', f'{prefix}interactions'
    input_k = 'input_rescaled' if 'input_rescaled' in result_dict else 'input_data'
    ...
    modules, tpcs = geo.get_contributors(particle.sources)
    ...
    for i, t in enumerate(tpcs):
        ...
        for sister in sisters:
            ...
            if truth:
                ...
                result_dict['cluster_label_adapted'][index, dcol] -= offsets[i]
...
def get_cathode_offsets(particle, geo, module, tpcs, truth_point_mode='points'):
    '''
    ...
    '''
    # Get the cathode position
    daxis, cpos = geo.cathodes[module]
    dvector = (np.arange(3) == daxis).astype(float)

    # Check which side of the cathode each TPC lives
    flip = (-1) ** (geo.boundaries[module, tpcs[0], daxis].mean() \
            > geo.boundaries[module, tpcs[1], daxis].mean())
    ...
    for i, t in enumerate(tpcs):
        ...
        index  = geo.get_tpc_index(particle.sources, module, t)
        points = get_points(particle, truth_point_mode)[index]
        idx0, idx1, _ = farthest_pair(points, 'recursive')
        end_points = points[[idx0, idx1]]

        # Find the point closest to the cathode
        tpc_offset = geo.get_tpc_offset(end_points, module, t)[daxis]
    ...
    #    vplane = dvector - end_dir/end_dir[daxis]
