...
sample = sys.argv[3]
job_name = sys.argv[4]
voxel_count_threshold = int(sys.argv[5])
...
systematics = ["nominal", "lyield", "sce_on", "modboxa_plus", "modboxa_minus",
                "modboxb_plus", "modboxb_minus",
                "ldiffusion_plus", "ldiffusion_minus",
                "tdiffusion_plus",
                "tdiffusion_minus"]
#systematics = ["modboxa_plus", "modboxa_minus", "modboxb_plus", "modboxb_minus"]
if task_id > len(systematics):
    exit(0)
systematic = systematics[task_id-1]

#input_files = io_cfg['iotool']['dataset']['data_keys']
sample_dir = os.path.join("/sdf/group/neutrino/ldomine/ICARUS_Analysis_MC", "corsika_systematics_%s_%s" % (systematic, sample))
...
io_cfg['iotool']['dataset']['limit_num_files'] = 300

# MAke output directory
#out_dir = analysis_cfg['analysis']['log_dir']
job_out_dir = os.path.join("/sdf/group/neutrino/ldomine/analysis/systematics", "log_corsika_systematics_%s_%s" % (sample, job_name))
job_out_dir = os.path.join(job_out_dir, "%s" % systematic)
