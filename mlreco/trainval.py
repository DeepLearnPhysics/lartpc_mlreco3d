...
class trainval(object):
    ...
    def _forward(self, data_blob):
        ...
        with torch.set_grad_enabled(self._train):
            ...
            tstart = time.time()
            net_out = self._net(data)

            # Compute the loss
            if loss_keys:
                loss_acc = self._criterion(net_out, *tuple([data_blob[key] for key in loss_keys]))
                if self._train:
            ...
            if 'analysis_keys' in self._model_config:
                for key in self._model_config['analysis_keys']:
                    #res[key] = [loss_acc[key]]
                    res[key] = [s.cpu().detach().numpy() for s in net_out[self._model_config['analysis_keys'][key]]]
            return res
