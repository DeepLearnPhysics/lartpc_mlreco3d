...
class PPN(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        for i in range(self._ppn_num_conv):
            self.ppn1_conv.add(scn.SubmanifoldConvolution(self._dimension, nPlanes[-1], nPlanes[-1], 3, False))
        ...
        for i in range(self._num_strides-self.half_stride-1):
    ...
    def forward(self, input):
        ...
        if use_encoding:
            feature_map1 = ppn1_feature_enc[-1]
            feature_map2 = ppn1_feature_enc[self.half_stride2]
            feature_map3 = ppn1_feature_enc[0]
        else:
            feature_map1 = ppn1_feature_dec[0]
            feature_map2 = ppn1_feature_dec[self.half_stride2]
            feature_map3 = ppn1_feature_dec[-1]
        ...
        if self._downsample_ghost:
            with torch.no_grad():
                ...
                feature_map1, ghost_mask1 = self.ghost_mask(ghost_mask, coords, feature_map1, factor=(self._num_strides-1))
                feature_map2, ghost_mask2 = self.ghost_mask(ghost_mask, coords, feature_map2, factor=self.half_stride)
...
class PPNLoss(torch.nn.modules.loss._Loss):
    def __init__(self, cfg, reduction='sum'):
        ...
        self._true_distance = self._cfg.get('true_distance', 1.0)
        ...
        for i in range(len(label)):
            ...
            for b in batch_ids[i].unique():
                ...
                if event_label.size(0) > 0:
                    ...
                    event_label_ppn2 = torch.floor(event_label/(2**self.half_stride))
                    d_true_ppn1 = self.distances(event_label_ppn1, event_ppn1_data)
                    d_true_ppn2 = self.distances(event_label_ppn2, event_ppn2_data)
                    positives_ppn1 = (d_true_ppn1 < self._true_distance).any(dim=0).long()
                    positives_ppn2 = (d_true_ppn2 < self._true_distance).any(dim=0).long()
                    ...
                    weight = torch.stack([w, 1-w]).double()

                    # loss_seg_ppn1 = torch.mean(self.cross_entropy(event_ppn1_scores.double(), positives_ppn1.long()))
                    loss_seg_ppn1 = torch.nn.functional.cross_entropy(event_ppn1_scores.double(), positives_ppn1, weight=weight)
