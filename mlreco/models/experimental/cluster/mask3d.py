...
class Mask3d(nn.Module):
    ...
    def __init__(self, cfg, name='mask3d'):
        ...
        self.mask_head = ME.MinkowskiConvolution(num_features, self.mask_dim, 
            kernel_size=1, stride=1, bias=True, dimension=self.D)

        
        # self.instance_to_mask = MLP([self.mask_dim] * 3)
        self.instance_to_mask = nn.Sequential(
            nn.Linear(self.mask_dim, self.mask_dim),
            nn.ReLU(),
            nn.Linear(self.mask_dim, self.mask_dim)
        )
        self.instance_to_class = nn.Sequential(
            nn.Linear(self.mask_dim, self.mask_dim),
            nn.ReLU(),
            nn.Linear(self.mask_dim, self.num_classes)
        )
        ...
        self.shared_decoders = self.model_config.get('shared_decoders', True)

        # Transformer Modules
        if self.shared_decoders:
            num_shared = 1
        else:
            num_shared = self.num_decoders

        self.transformers = nn.ModuleList()

        for num_trans in range(num_shared):
            self.transformers.append(TransformerDecoder(self.mask_dim, 
                                                        self.num_heads, 
    ...
    def get_positional_encoding(self, x):
        ...
        for i in range(len(x.decomposed_coordinates)):
            coords = x.decomposed_coordinates[i]
            pos_enc_batch = self.query_module.pos_enc(coords, features=None)
            pos_encoding.append(pos_enc_batch)

        pos_encoding = torch.cat(pos_encoding, dim=0)
        return pos_encoding
    ...
    def forward(self, point_cloud):
        ...
        for tf_index in range(self.num_transformers):
            ...
            for i, fmap in enumerate(decoderOutput):
                assert queries.shape == (batch_size, self.query_module.num_queries, self.mask_dim)
                ...
                output = self.transformers[transformer_index](queries, 
                    query_pos,
                    src_pcd, 
                    batched_pos_enc, 
                    batched_attn)
