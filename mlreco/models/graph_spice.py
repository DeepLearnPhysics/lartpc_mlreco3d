...
class GraphSPICEPP(nn.Module):
    ...
    def __init__(self, cfg, name='graph_spice'):
        super(GraphSPICEPP, self).__init__()
        self.model_config = cfg[name]
        self.skip_classes = self.model_config.get('skip_classes', [2, 3, 4])
        self.dimension = self.model_config.get('dimension', 3)
    ...
    def forward(self, input):
        ...
        if self.use_raw_features:
            ...
            temp = res['hypergraph_features'][0]
            tangents = res['tangents'][0]
            hypergraph_features = torch.cat([coordinates, tangents, temp], dim=1)
...
class GraphSPICEPPLoss(GraphSPICELoss):

    def __init__(self, cfg, name='spice_loss'):
        super(GraphSPICEPPLoss, self).__init__(cfg)
        self.vector_loss_fn = VectorEstimationLoss(self.loss_config)
        self.mincut_loss_fn = None
    ...
    def forward(self, result, segment_label, cluster_label):
        ...
        vec_loss = self.vector_loss_fn(graph)
        print("Vector Loss = ", vec_loss)
        res['loss'] += vec_loss
