...
def get_ppn_points_per_particles(input_data, res,
    ...
    ppn_candidates, c_candidates, distances = [], [], []
    ...
    if return_distances:
        return ppn_candidates, c_candidates, distances
    else:
...
def predict_vertex(inter_idx, data_idx, input_data, res,
                    ...
                    other_primaries_threshold=30, fraction_bad_primaries=0.6,
                    pca_radius=21):
    """
    ...
    if return_distances:
        ppn_candidates, c_candidates, distances = out
    else:
        ppn_candidates, c_candidates = out
    ...
    if len(ppn_candidates) > 1:
        ...
        for p_idx, points in enumerate(ppn_candidates):
            ...
            for c_idx, c2 in enumerate(c_candidates):
                ...
                d3 = scipy.spatial.distance.cdist(points[:, coords_col[0]:coords_col[1]], all_voxels[c2, coords_col[0]:coords_col[1]])
            ...
            #if (distance_to_other_primaries >= other_primaries_threshold).sum() >= np.ceil(fraction_bad_primaries * len(distance_to_other_primaries)):
            # if (distance_to_other_primaries >= other_primaries_threshold).all():
            ...
            #    continue
            if (other_primaries_threshold > -1) and (distance_to_other_primaries.min() >= other_primaries_threshold):
               print("Skipping", p_idx, (distance_to_other_primaries >= other_primaries_threshold).sum(), len(distance_to_other_primaries), distance_to_other_primaries)
               continue
            ...
            if best_candidate_distance < inter_threshold:
                ...
                d = scipy.spatial.distance.cdist(all_voxels[c_candidates[p_idx], coords_col[0]:coords_col[1]], [points[candidate_distances.argmin(), coords_col[0]:coords_col[1]]])
                ...
                print("candidate ", ppn_candidates2[-1][:, 1:4], np.sum(np.abs(points_to_other_primaries), axis=1).min())
        ...
        if len(ppn_candidates2) > 1:
            ...
            for p1_idx in range(0, len(ppn_candidates2)):
                for p2_idx in range(p1_idx+1, len(ppn_candidates2)):
                    closest_points.extend(find_closest_points_of_approach(ppn_candidates2[p1_idx, coords_col[0]:coords_col[1]],
                                                    directions[p1_idx],
                                                    ppn_candidates2[p2_idx, coords_col[0]:coords_col[1]],
                                                    directions[p2_idx]))
