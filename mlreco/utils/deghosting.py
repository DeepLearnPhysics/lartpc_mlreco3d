...
#         for batch_id in coords[:, batch_column].unique():
...
#             mask = nonghost_mask & (label_seg[i][:, batch_column][batch_mask] == num_classes)
#             # Assign them to closest cluster
#             d = distances(batch_coords[mask, c1:c2], batch_clustering[:, c1:c2]).argmin(dim=1)
...
#             new_label_clustering[label_seg[i][batch_mask, batch_column] < num_classes] = batch_clustering
